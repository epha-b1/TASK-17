...
func (s *Service) QueueBookingConfirmed(ctx context.Context, tx pgx.Tx, bookingID string, at time.Time) error {
	var topicID string
	err := tx.QueryRow(ctx, `SELECT id::text FROM notification_topics WHERE name='booking_success' LIMIT 1`).Scan(&topicID)
	if err != nil {
		return err
	}
	...
	for _, userID := range userIDs {
		...
		err = tx.QueryRow(ctx, `
			...
			VALUES ($1::uuid, $2::uuid, 'Booking confirmed', 'Your booking has been confirmed.', $3::uuid, $4)
			RETURNING id::text
		`, userID, topicID, bookingID, at.UTC()).Scan(&notificationID)
		if err != nil {
			return err
		}
		...
		payload, _ := json.Marshal(map[string]any{"recipient": userID, "booking_id": bookingID})
		_, err = tx.Exec(ctx, `
		...
		`, notificationID, userID, bookingID, topicID, status, nextAttemptAt, string(payload), at.UTC(), deliveredAt)
		if err != nil {
			return err
		}
	}
